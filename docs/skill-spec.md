...
version: 0.4.0
description: "Use this Agent Skill whenever you need to prepare, validate, publish, update, or remove Sphere fragments — or query earnings, consumption analytics, and payment settings. Sphere is an agent-readable publishing and access layer for structured, licensed content. Use this skill when the user says 'crea una fragment Sphere', 'prepara questo contenuto per Sphere', 'pubblica su Sphere', 'trasforma questa cartella in Sphere', 'quanto ho guadagnato', 'collega Stripe', or any request related to publishing, monitoring, or monetizing content on Sphere. Always show a human-readable preview for user confirmation before any write or publish operation. Consumers of Sphere content are always LLMs, agents, or automated systems, never humans."
---
...
Sphere is a headless publishing and access layer for LLM distribution. Publishers author minimal content packages (fragments). Sphere ingests, transforms, and exposes them via HTTP to agent consumers. Consumers pay per access or via subscription. Publishers receive revenue share automatically via Stripe.
